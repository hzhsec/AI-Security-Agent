...
- **外部参考资料导入**：可以先在网上搜索工具用法，然后粘贴到系统中。系统会自动清理格式（去除HTML标签等）、提取命令示例，AI 学习时会参考这些资料
- **重新探索**：对已有知识条目点击"重新探索"，可用最新版本覆盖旧知识
- **手动管理**：支持手动添加、补充、删除知识条目；支持一键清空全部知识库
- **导入导出**：支持将工具知识导出为 JSON 文件备份，或导入已有知识库，避免重复学习
- **编辑工具信息**：支持修改工具路径、简介等，适应工具迁移场景

#### 内置工具知识库

系统预置了两个经典 Linux 安全应急响应工具的知识库，开箱即用：

| 工具 | 项目地址 | 功能描述 |
| ---- | -------- | -------- |
| **whohk** | [wgpsec/whohk](https://github.com/wgpsec/whohk) | Linux 应急响应工具，支持用户账户检查、历史命令审查、定时任务分析、网络连接监控、SSH 登录审计、文件变更检测、恶意样本检测（YARA 规则）、WebShell 排查等 |
| **Linuxgun** | [sun977/Linuxgun](https://github.com/sun977/linuxcheckshoot) | Linux 安全应急响应检查脚本（原 linuxcheck），支持系统信息排查、网络连接分析、进程排查、文件系统检查、后门与攻击痕迹检测（SUID/SGID、隧道特征、WebShell）、Kubernetes 安全支持等 |

这些工具已内置使用方法说明，AI 可直接调用进行安全巡检任务。

#### 外部参考资料导入

如果你想让 AI 学习一个工具的用法，可以先在网上搜索该工具的使用说明，然后把搜索结果粘贴进来：

1. 在网上搜索工具使用方法（如百度、Google、CSDN 等）
2. 复制搜索结果（可能带有HTML标签、杂乱格式）
3. 打开 **工具知识库** 标签页，点击 **导入参考资料**
4. 粘贴内容，选择对应的工具名
5. 系统自动清理格式、提取命令示例
6. 然后点击 **让AI自学**，AI 会结合你提供的资料进行学习

导入的参考资料会被保存，后续随时可以基于这些资料让 AI 重新学习。
...
├── tool_knowledge.py   # 工具知识库（错误记录、用法总结、AI自学、导入导出）
...
| `GET`    | `/tool-knowledge/export`                   | 导出全部工具知识库               |
| `GET`    | `/tool-knowledge/export/{tool_name}`       | 导出指定工具知识                 |
| `POST`   | `/tool-knowledge/import`                   | 导入工具知识                     |
| `POST`   | `/tool-knowledge/reference`                | 导入网上搜索的参考资料            |
| `GET`    | `/tool-knowledge/reference/{tool_name}`    | 获取工具的参考资料               |
...
- 新增工具知识库导入导出功能
- 新增工具信息编辑功能（支持修改路径、简介等）
- 优化未配置模型时的提示显示
- 修复 API Key 可能错误显示的问题
- 减少 README 中 emoji 的使用
